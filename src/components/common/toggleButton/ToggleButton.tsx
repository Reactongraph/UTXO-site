...
import { setCookieItem } from '@/utils/helpers/cookieHelper';
...
// Define the styled components as before
...
const ToggleButton: React.FC = () => {
  const { theme, toggleTheme } = useThemeContext() as ThemeContext;
  const [switchState, setSwitchState] = React.useState<boolean>(theme === 'dark');
  ...
  return (
    <div style={{ position: 'relative' }}>
      <FormGroup>
        <FormControlLabel
          id={windowWidth > 756 ? 'switch-toggle' : 'toggle'}
          control={
            windowWidth > 756 ? (
              <MaterialUISwitch sx={{ m: 1 }} checked={switchState} onChange={handleSwitchChange} />
            ) : (
              <MaterialUISwitchSmall checked={switchState} onChange={handleSwitchChange} />
            )
          }
          label={undefined}
        />
